...
public class FCallDialog extends javax.swing.JDialog {
     ...
     */
    private static Element cfg;

    private static class FCallPanelHolder {

        private static final FCallDialog INSTANCE = new FCallDialog(prnt, false);
    }
    static private java.awt.Frame prnt;
    private static int delay;

    public static FCallDialog getInstance(java.awt.Frame parent, Element mainElement) {
        prnt = parent;
        cfg = mainElement;
        ...
        return FCallPanelHolder.INSTANCE;
    }
    ...
    public void show(String moner, String point) {
        ...
        bq.add(template.replaceFirst("###", moner).replaceFirst("@@@", point));
    }
    final private BlockingQueue<String> bq = new LinkedBlockingQueue<>();

    /** Creates new form FCallDialog */
    private FCallDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        setLocation(parent.getX() + Integer.parseInt(Uses.elementsByAttr(cfg, Uses.TAG_BOARD_NAME, Uses.TAG_BOARD_CALL_PANEL_X).get(0).attributeValue(Uses.TAG_BOARD_VALUE)),
                parent.getY() + Integer.parseInt(Uses.elementsByAttr(cfg, Uses.TAG_BOARD_NAME, Uses.TAG_BOARD_CALL_PANEL_Y).get(0).attributeValue(Uses.TAG_BOARD_VALUE)));
