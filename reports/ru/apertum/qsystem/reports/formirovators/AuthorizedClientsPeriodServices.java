...
public class AuthorizedClientsPeriodServices extends AFormirovator {
    ...
    public byte[] preparation(String driverClassName, String url, String username, String password, String inputData) {
        ...
        if (flag) {
            ...
            result = result.replaceFirst(Uses.ANCHOR_DATA_FOR_REPORT, subject).replaceFirst(Uses.ANCHOR_ERROR_INPUT_DATA, mess).replaceFirst("#DATA_FOR_TITLE#", "Отчет по авторизованным персонам за период для услуги:").replaceFirst("#DATA_FOR_USERS#", users_select);
            try {
