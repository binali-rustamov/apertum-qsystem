...
/*
    ...
    public byte[] preparation(String driverClassName, String url, String username, String password, HttpRequest request) {
        ...
        if ("".equals(data)) {
            ...
            if (ss.length == 3) {
                ...
                try {
                    ...
                    user_id = Integer.parseInt(ss1[1]);
                    user = ss2[1];
                } catch (Exception ex) {
                ...
                if (!flag) {
                    ...
                    paramMap.put(ss1[0], new Integer(user_id));
                    paramMap.put(ss2[0], user);
                }
            } else {
        ...
        if (flag) {
            ...
            final InputStream inStream = getClass().getResourceAsStream("/ru/apertum/qsystem/reports/web/get_date_distribution_users.html");
            String result = null;
            String users_select = "";
            try {
            ...
            result = result.replaceFirst(Uses.ANCHOR_DATA_FOR_REPORT, request.getRequestLine().getUri()).replaceFirst(Uses.ANCHOR_ERROR_INPUT_DATA, mess).replaceFirst("#DATA_FOR_TITLE#", "Распределение среднего времени ожидания внутри дня для пользователя:").replaceFirst("#DATA_FOR_USERS#", users_select);
            try {
