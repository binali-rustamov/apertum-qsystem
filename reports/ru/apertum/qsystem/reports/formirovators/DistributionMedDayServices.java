...
public class DistributionMedDayServices extends AFormirovator {
    ...
    public byte[] preparation(String driverClassName, String url, String username, String password, String inputData) {
        ...
        if (flag) {
            ...
            result = result.replaceFirst(Uses.ANCHOR_DATA_FOR_REPORT, subject).replaceFirst(Uses.ANCHOR_ERROR_INPUT_DATA, mess).replaceFirst("#DATA_FOR_TITLE#", "Отчет предварительно зарегистрированных клиентов по услуге на дату:").replaceFirst("#DATA_FOR_SERVICES#", services_select);
            try {
